    ...
    pub fn execute_parallel(
        ...
        match range_expr {
            Expr::Number(n) => {
                ...
                println!(
                    "[GPU] Allocating {} threads on simulated GPU",
                ...
                let handles: Vec<_> = (0..self.parallel_threads.min(n as usize))
                    .map(|thread_id| {
                        ...
                        let chunk_size =
                            (n as usize + self.parallel_threads - 1) / self.parallel_threads;
                        let start = thread_id * chunk_size;
                        let end = std::cmp::min(start + chunk_size, n as usize);

                        println!(
                            "[GPU] Thread {} processing iterations {}..{}",
                            thread_id, start, end
                        );
            ...
            Expr::Range { start, end } => match (&**start, &**end) {
                (Expr::Number(start_val), Expr::Number(end_val)) => {
                    println!(
                        "[GPU] Simulating parallel loop over range {}..{}",
                        start_val, end_val
                    );
